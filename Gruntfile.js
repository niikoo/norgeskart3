module.exports = function (grunt) {
  ...
  function filterForCSS(files) {
    return files.filter(file => file.match(/\.css$/));
  }
  ...
  grunt.registerMultiTask('index', 'Process index.html template', function () {
    const jsFiles = filterForJS(this.filesSrc)
      .filter(noAngularDependenciesFile)
      .map(mapDirectoriesToTargetEnvironment);
    const cssFiles = filterForCSS(this.filesSrc)
      .map(mapDirectoriesToTargetEnvironment);
  ...
  grunt.registerMultiTask('nugetadminidx', 'Process index.cshtml template for nuget', () => {
    // var markup = grunt.file.read("src/html/configMarkup.html");
    const packageName = 'mapAdminTool';
    const packageNameUcfirst = ucFirst(packageName);
    const copyOptions = {
      process(contents) {
        return grunt.template.process(contents, {
          data: {
    ...
    const dest = `${grunt.config('nuget_dir')}/Views/${packageNameUcfirst}Page/`;
  ...
  // build .nuspec file
  grunt.registerMultiTask('nuspec', 'create .nuspec file', () => {
    const files = readDir(grunt.config('nuget_dir'));
    ...
    const dest = `${grunt.config('nuget_dir')}/`;
  ...
  grunt.registerMultiTask('nugetcontroller', 'Create controller', function () {
    const excludes = grunt.config('vendor_nuget_excludes');
    const files = _.filter(this.filesSrc, file => (excludes.indexOf(file) === -1));

    const jsFiles = getfilenames(filterForJS(files));
    const cssFiles = getfilenames(filterForCSS(files));
    // cssFiles = cssFiles.concat(fs.readdirSync('sass/css'));
    const packageName = grunt.config('pkg.name');
    const copyOptions = {
      process(contents) {
        return grunt.template.process(contents, {
          data: {
    ...
    const dest = `${grunt.config('nuget_dir')}/Controllers/${ucFirst(packageName)}PageController.cs`;
