  ...
  function ($scope, mainAppService, $http, searchKoordTransPanelFactory) {
    ...
    var _addSearchOptionToPanel = function (data) {
      if ($scope.activePosition.resSosiKoordSys == '50' || $scope.activePosition.resSosiKoordSys == '84') {
        ...
        $scope.activePosition.transLon2 = deg + '\xb0 ' + _round(rest, 7) + '\'';
        var min = Math.floor(rest);
        var sec = (rest - min) * 60;
        $scope.activePosition.transLon3 = deg + '\xb0 ' + min + '\' ' + _round(sec, 5) + '\'\'';
        deg = Math.floor(data.nord);
        rest = (data.nord - deg) * 60;
        $scope.activePosition.transLat2 = deg + '\xb0 ' + _round(rest, 7) + '\'';
        min = Math.floor(rest);
        sec = (rest - min) * 60;
        $scope.activePosition.transLat3 = deg + '\xb0 ' + min + '\' ' + _round(sec, 5) + '\'\'';
      } else {
